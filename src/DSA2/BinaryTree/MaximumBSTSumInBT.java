...
class Solution {
    ...
    public Data solve(TreeNode root, int[] res) {
        if (root == null)
            return new Data(Integer.MAX_VALUE, Integer.MIN_VALUE, 0, true);
        ...
        if (left.isBST && right.isBST && root.val > left.max && root.val < right.min) {
            ...
            int minVal = Math.min(root.val, left.min); // most important micros
            int maxVal = Math.max(root.val, right.max);

            return new Data(minVal, maxVal, currSum, true);
        }

        return new Data(-1, -1, 0, false);
    }
}
